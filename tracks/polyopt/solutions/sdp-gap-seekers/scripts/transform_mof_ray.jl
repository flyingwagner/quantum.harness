...
function main_forward(args=ARGS)
    length(args) == 3 || error(
        "usage: transform_mof_ray.jl SCALE.tsv ORIGINAL.variables.tsv SCALED.variables.tsv",
    )
    scale_indices, scale_names, scales = read_scale_map(args[1])
    ray_indices, original_values = read_ray_values(args[2])
    ...
    values = transform_ray(original_values, scales)
    ...
    println("original_ray_scale\t", maximum(abs, original_values))
    println("scaled_ray_scale\t", maximum(abs, values))
